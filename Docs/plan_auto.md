...
output_dir = data_root / sub_dir / Slicer / dataset_name
video      = data_root / sub_dir / Slicer / dataset_name / slicer.mp4
```
...
output_dir = <repo>/data/0630/Slicer/QPA-2026-06-30-161131
video      = <repo>/data/0630/Slicer/QPA-2026-06-30-161131/slicer.mp4
```
...
output_dir = X:/关键帧/0630/Slicer/QPA-2026-06-30-161131
video      = X:/关键帧/0630/Slicer/QPA-2026-06-30-161131/slicer.mp4
```
...
```text
data_root / sub_dir / Slicer / dataset_name / slicer.mp4 存在，且文件大小 > 0
```
...
- 过滤掉已经存在 `data_root/0630/Slicer/<dataset_name>/slicer.mp4` 的数据集。
- 从未完成数据集中选择最新一个执行。
...
- 如果未传 `--output_dir`，输出到 `input_dir.parent / "Slicer" / input_dir.name`。
...
def dataset_output_dir(input_dir: Path) -> Path:
    return input_dir.parent / "Slicer" / input_dir.name
...
output_dir = input_dir.parent / "Slicer" / input_dir.name
```
...
项目内日常处理最新新数据，JSON 中 `data_root = "data"`：
...
```text
<repo>/data/0630/Slicer/QPA-2026-06-30-161131/slicer.mp4
```
...
```text
<repo>/data/0630/Slicer/QPB-2026-06-30-172000/slicer.mp4
```

共享盘日常处理最新新数据，JSON 中 `data_root = "X:/关键帧"`：
...
```text
X:/关键帧/0630/Slicer/<dataset_name>/slicer.mp4
```
...
- `data_root = data` 且 `--sub_dir 0630`：
  ...
  - 跳过已有 `<repo>/data/0630/Slicer/<dataset_name>/slicer.mp4` 的目录。
  - 只选择最新未完成数据集。
- `data_root = X:/关键帧` 且 `--sub_dir 0630`：
  - 使用同一套逻辑构建 `X:/关键帧/0630/Slicer/<dataset_name>/slicer.mp4`。
- `--sub_dir 0630 --force`：
  - 选择该批次下所有数据集。
...
- `-s X:/关键帧/0630/QPA-2026-06-30-161131`：
  - 默认输出为 `X:/关键帧/0630/Slicer/QPA-2026-06-30-161131/slicer.mp4`。
...
- `--sub_dir` 默认只处理最新未完成数据，避免一次性处理大量积压数据。
- 如果需要处理多个未完成数据集，可后续增加 `--all_new`；第一版先按“最新新数据”实现。
